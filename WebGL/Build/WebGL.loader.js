function createUnityInstance(t, r, d) {
    ...
    for (o in e(r, "companyName", "Unity"), e(r, "productName", "WebGL Player"), e(r, "productVersion", "1.0"), r)
        c[o] = r[o];
    ...
    c.deinitializers.push(function () {
        for (var e in c.disableAccessToMediaDevices(), a.forEach(function (e) {
            ...
            }), window.removeEventListener("error", n), window.removeEventListener("unhandledrejection", n), c.intervals)
            window.clearInterval(e);
        c.intervals = {}
    }),
    ...
    if ("symbolsUrl" != e) {
        ...
        for (e in c.downloadProgress) {
            if (!(r = c.downloadProgress[e]).started)
                return;
            a++,
            r.lengthComputable ? (n += r.loaded, o += r.total, s++) : r.finished || i++
        }
            ...
            if ("undefined" == typeof unityFramework || !unityFramework) {
                ...
                for (e in t) {
                    ...
                    if (c.frameworkUrl.endsWith("." + n[0]))
                        return r = "Unable to parse " + c.frameworkUrl + "!", "file:" == location.protocol ? void i(r + " Loading pre-compressed (brotli or gzip) content via a file:// URL without a web server is not supported by this browser. Please use a local development web server to host compressed Unity content, or use the Unity Build and Run option.", "error") : (r += ' This can happen if build compression was enabled but web server hosting the content was misconfigured to not serve the file with HTTP Response Header "Content-Encoding: ' + n[1] + '" present. Check browser Console and Devtools Network tab to debug.', "br" == n[0] && "http:" == location.protocol && (n = -1 != ["localhost", "127.0.0.1"].indexOf(location.hostname) ? "" : "Migrate your server to use HTTPS.", r = /Firefox/.test(navigator.userAgent) ? "Unable to parse " + c.frameworkUrl + '!<br>If using custom web server, verify that web server is sending .br files with HTTP Response Header "Content-Encoding: br". Brotli compression may not be supported in Firefox over HTTP connections. ' + n + ' See <a href="https://bugzilla.mozilla.org/show_bug.cgi?id=1670675">https://bugzilla.mozilla.org/show_bug.cgi?id=1670675</a> for more information.' : "Unable to parse " + c.frameworkUrl + '!<br>If using custom web server, verify that web server is sending .br files with HTTP Response Header "Content-Encoding: br". Brotli compression may not be supported over HTTP connections. Migrate your server to use HTTPS.'), void i(r, "error"))
                }
    ...
    c.preRun.push(function () {
        ...
        o.then(function (e) {
            ...
            for (r += 4; r < o; ) {
                ...
                r += i;
                for (var l = 0, u = d.indexOf("/", l) + 1; 0 < u; l = u, u = d.indexOf("/", l) + 1)
...
return c.SystemInfo = function () {
    ...
    for (var d = [["Windows (.*?)[;)]", "Windows"], ["Android ([0-9_.]+)", "Android"], ["iPhone OS ([0-9_.]+)", "iPhoneOS"], ["iPad.*? OS ([0-9_.]+)", "iPadOS"], ["FreeBSD( )", "FreeBSD"], ["OpenBSD( )", "OpenBSD"], ["Linux|X11()", "Linux"], ["Mac OS X ([0-9_.]+)", "MacOS"], ["bot|google|baidu|bing|msn|teoma|slurp|yandex", "Search Bot"]], l = 0; l < d.length; ++l)
        if (u = s(d[l][0], o, 1)) {
            r = d[l][1],
            u = u.replace(/_/g, ".");
            break
...
c.fetchWithProgress = function (e, t) {
    var c = function () {};
    return t && t.onProgress && (c = t.onProgress),
        ...
        function o() {
            return void 0 === e ? t.arrayBuffer().then(function (e) {
                ...
                if (e.done) {
                    ...
                    if (l < s)
                        return i.slice(0, l);
                    for (var t = new Uint8Array(l), r = (t.set(i, 0), u), n = 0; n < d.length; ++n)
                        t.set(d[n], r), r += d[n].length;
                    return t
                }
                return l + e.value.length <= i.length ? (i.set(e.value, l), u = l + e.value.length) : d.push(e.value),
                l += e.value.length,
                c({
                    type: "progress",
                    total: Math.max(s, l),
...
c.cachedFetch = (g = c.UnityCache, m = g.RequestStore, b = c.fetchWithProgress, P.prototype.arrayBuffer = function () {
    ...
    function i(e, t) {
        return b(e, t).then(function (e) {
            if (s.enabled && !s.revalidated) {
                if (304 === e.status)
                    return s.result.revalidated = s.result.accessed, s.revalidated = !0, a.storeRequest(s.result).then(function () {
    ...
    return o && (s.control = o.control, s.company = o.company, s.product = o.product),
